...
class RRTStarTorque:
    def __init__(self, start, goal, obstacles, robot, area, max_iter=500, step_size=0.1, dt=0.01):
    ...
    def nearest_node(self, q):
        return min(self.nodes, key=lambda n: np.linalg.norm(n[0][:2] - q[:2]))
    ...
    def plan(self):
        for _ in range(self.max_iter):
            ...
            torque_cost = self.torque_cost(q_nearest[0], q_new)
            new_cost = q_nearest[1] + torque_cost
